...
export function VenueDetailPage() {
  ...
  return <section className="section page-section profile-detail">
    ...
    <div className="detail-dashboard"><div className="dashboard-main"><VenueMap name={venue.name} address={venue.road_address || [venue.province, venue.district, venue.address_detail].filter(Boolean).join(' ')} latitude={venue.latitude} longitude={venue.longitude} geocodeStatus={venue.geocode_status} /><div className="section-heading compact-heading venue-record-heading"><h2>최근 공연</h2></div><RecordList items={setlists} /></div><aside className="stats-rail"><section><h2>주요 아티스트</h2>{artists.length ? <ol className="ranking-list">{artists.map(([name, count]) => <li key={name}><span>{name}</span><strong>{count}회</strong></li>)}</ol> : <p className="muted-copy">첫 기록이 쌓이면 이 공연장을 찾은 아티스트를 보여드려요.</p>}</section></aside></div>
  </section>;
}
