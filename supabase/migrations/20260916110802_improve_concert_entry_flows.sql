-- A scheduled concert may have no setlist yet, but can carry a verified ticket
-- link. Songs retain an optional exact YouTube URL; no platform key is exposed.
alter table public.concerts
  add column if not exists ticket_url text,
  add constraint concerts_ticket_url_check check (ticket_url is null or ticket_url ~* '^https?://');

alter table public.songs
  add column if not exists youtube_url text,
  add constraint songs_youtube_url_check check (youtube_url is null or youtube_url ~* '^https?://(?:www\.)?(youtube\.com|youtu\.be)/');

-- Remove only completely detached artifacts left by deleted test records. An
-- artist still used by a concert, song, or cover credit is never removed.
delete from public.metadata_jobs j
where j.entity_type = 'artist'
  and j.entity_id in (
    select a.id from public.artists a
    where not exists (select 1 from public.concerts c where c.artist_id = a.id)
      and not exists (select 1 from public.songs s where s.artist_id = a.id or s.original_artist_id = a.id)
  );
...
join public.concerts c on c.artist_id = a.id
...
select
  ss.id, ss.setlist_id, ss.song_id, coalesce(song.title::text, ss.custom_title) as title,
  ss.section, ss.position, ss.is_cover, original.name::text as original_artist_name,
  ss.guest_artist, ss.note, ss.custom_title, album.name::text as album_name,
  album.image_url as album_image_url, album.release_date, song.external_track_id,
...
drop function public.create_setlist(text, date, text, text, text, text, text, text, text, jsonb);
drop function public.replace_setlist(uuid, text, date, text, text, text, text, text, text, text, jsonb);

create function public.create_setlist(
...
begin
  ...
  if nullif(trim(coalesce(p_venue_name, '')), '') is not null then
    ...
    if v_venue_id is null then
      ...
      update public.venues set road_address = coalesce(nullif(trim(p_road_address), ''), road_address), province = coalesce(nullif(trim(p_province), ''), province), district = coalesce(nullif(trim(p_district), ''), district), address_detail = coalesce(nullif(trim(p_address_detail), ''), address_detail) where id = v_venue_id;
    end if;
  end if;
  ...
  for v_item in select value, ordinality from jsonb_array_elements(p_songs) with ordinality loop
    ...
    if v_song_id is null then insert into public.songs (artist_id, title, original_artist_id, youtube_url) values (v_artist_id, trim(v_item.value ->> 'title'), v_original_artist_id, nullif(trim(v_item.value ->> 'youtube_url'), '')) returning id into v_song_id; else update public.songs set youtube_url = coalesce(nullif(trim(v_item.value ->> 'youtube_url'), ''), youtube_url) where id = v_song_id; end if;
...
create function public.replace_setlist(
...
begin
  ...
  if nullif(trim(coalesce(p_venue_name, '')), '') is not null then
    select id into v_venue_id from public.venues where name = trim(p_venue_name) and (road_address is not distinct from nullif(trim(p_road_address), '') or (province is not distinct from nullif(trim(p_province), '') and district is not distinct from nullif(trim(p_district), ''))) limit 1;
    if v_venue_id is null then insert into public.venues (name, road_address, province, district, address_detail) values (trim(p_venue_name), nullif(trim(p_road_address), ''), nullif(trim(p_province), ''), nullif(trim(p_district), ''), nullif(trim(p_address_detail), '')) returning id into v_venue_id;
    else update public.venues set road_address = coalesce(nullif(trim(p_road_address), ''), road_address), province = coalesce(nullif(trim(p_province), ''), province), district = coalesce(nullif(trim(p_district), ''), district), address_detail = coalesce(nullif(trim(p_address_detail), ''), address_detail) where id = v_venue_id; end if;
  end if;
